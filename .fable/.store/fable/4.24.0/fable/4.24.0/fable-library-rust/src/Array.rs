pub mod Array_ {
    use super::*;
    use crate::Global_::SR::arrayWasEmpty;
    use crate::Global_::SR::arraysHadDifferentLengths;
    use crate::Global_::SR::indexOutOfBounds;
    use crate::Global_::SR::inputMustBeNonNegative;
    use crate::Global_::SR::inputMustBePositive;
    use crate::Global_::SR::inputSequenceEmpty;
    use crate::Global_::SR::inputSequenceTooLong;
    use crate::Global_::SR::keyNotFoundAlt;
    use crate::Global_::SR::notAPermutation;
    use crate::Global_::SR::notEnoughElements;
    use crate::Global_::SR::outOfRange;
    ...
    use crate::NativeArray_::reverse as reverse_1;
    use crate::NativeArray_::sortBy as sortBy_1;
    use crate::NativeArray_::toArray;
    ...
    pub fn reverse<T: Clone + 'static>(source: Array<T>) -> Array<T> {
        let res: Array<T> = toArray(source);
        reverse_1(res.clone());
        res
    }
    pub fn fill<T: Clone + 'static>(target: Array<T>, targetIndex: i32, count: i32, value: T) {
        if if (targetIndex) < 0_i32 {
            ...
            ((targetIndex) + (count)) > (get_Count(target.clone()))
        } {
            panic!(
                "{}",
        ...
        {
            let len: i32 = get_Count(target.clone());
    ...
    pub fn getSubArray<T: Clone + 'static>(
        ...
        if if (startIndex) < 0_i32 {
            ...
            ((startIndex) + (count)) > (get_Count(source.clone()))
        } {
            panic!(
                "{}",
                append_1((indexOutOfBounds()), string(" (Parameter \'index\')")),
    ...
    pub fn exactlyOne<T: Clone + 'static>(source: Array<T>) -> T {
        if (get_Count(source.clone())) == 1_i32 {
            source[0_i32].clone()
        } else {
            if source.is_empty() {
                panic!(
                    "{}",
                    append_1((inputSequenceEmpty()), string(" (Parameter \'array\')")),
                )
            } else {
                panic!(
                    "{}",
                    append_1((inputSequenceTooLong()), string(" (Parameter \'array\')")),
                )
            }
        }
    }
    ...
    pub fn head<T: Clone + 'static>(source: Array<T>) -> T {
        if source.is_empty() {
            panic!(
                "{}",
                append_1((arrayWasEmpty()), string(" (Parameter \'array\')")),
            )
        } else {
            source[0_i32].clone()
        }
    ...
    pub fn last<T: Clone + 'static>(source: Array<T>) -> T {
        let len: i32 = get_Count(source.clone());
        if source.is_empty() {
            panic!(
                "{}",
                append_1((arrayWasEmpty()), string(" (Parameter \'array\')")),
    ...
    pub fn tail<T: Clone + 'static>(source: Array<T>) -> Array<T> {
        if source.is_empty() {
            panic!(
                "{}",
                append_1((notEnoughElements()), string(" (Parameter \'array\')")),
            );
        }
        Array_::getSubArray(source.clone(), 1_i32, (get_Count(source)) - 1_i32)
    }
    pub fn append<T: Clone + 'static>(source1: Array<T>, source2: Array<T>) -> Array<T> {
        let len1: i32 = get_Count(source1.clone());
        let len2: i32 = get_Count(source2.clone());
    ...
    pub fn choose<T: Clone + 'static, U: Clone + 'static>(
        ...
        for i in 0_i32..=(get_Count(source.clone())) - 1_i32 {
    ...
    pub fn equals<T: PartialEq + Clone + 'static>(source1: Array<T>, source2: Array<T>) -> bool {
        let len1: i32 = get_Count(source1.clone());
        if (len1) == (get_Count(source2.clone())) {
            let i: MutCell<i32> = MutCell::new(0_i32);
    ...
    pub fn mapIndexed2<T1: Clone + 'static, T2: Clone + 'static, U: Clone + 'static>(
        ...
        if (get_Count(source1.clone())) != (get_Count(source2.clone())) {
            panic!("{}", arraysHadDifferentLengths(),);
        }
        {
            let len: i32 = get_Count(source1.clone());
            let res: Array<U> = new_with_capacity::<U>(len);
            for i in 0_i32..=(len) - 1_i32 {
                add(
                    res.clone(),
    ...
    >(
        ...
        if if (get_Count(source1.clone())) != (get_Count(source2.clone())) {
            true
        } else {
            (get_Count(source2.clone())) != (get_Count(source3.clone()))
        } {
            panic!("{}", arraysHadDifferentLengths(),);
        }
        {
    ...
    pub fn concat<T: Clone + 'static>(sources: Array<Array<T>>) -> Array<T> {
        ...
        for idx in 0_i32..=(get_Count(sources.clone())) - 1_i32 {
            let arr: Array<T> = sources[idx].clone();
            len.set((len.get().clone()) + (get_Count(arr)))
        }
        {
            let res: Array<T> = new_with_capacity::<T>(len.get().clone());
            for idx_1 in 0_i32..=(get_Count(sources.clone())) - 1_i32 {
                let arr_1: Array<T> = sources[idx_1].clone();
                for idx_2 in 0_i32..=(get_Count(arr_1.clone())) - 1_i32 {
    ...
    pub fn exists<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> bool {
        ...
        while if (i.get().clone()) < (get_Count(source.clone())) {
            !res.get().clone()
        } else {
            false
        } {
    ...
    pub fn exists2<T1: Clone + 'static, T2: Clone + 'static>(
        ...
        }
        {
            let i: MutCell<i32> = MutCell::new(0_i32);
            let res: MutCell<bool> = MutCell::new(false);
            while if (i.get().clone()) < (get_Count(source1.clone())) {
                !res.get().clone()
            } else {
                false
            } {
    ...
    pub fn initialize<T: Clone + 'static>(count: i32, initializer: Func1<i32, T>) -> Array<T> {
        if (count) < 0_i32 {
            panic!(
                "{}",
                append_1((inputMustBeNonNegative()), string(" (Parameter \'count\')")),
            );
        }
        {
            let res: Array<T> = new_with_capacity::<T>(count);
    ...
    pub fn pairwise<T: Clone + 'static>(source: Array<T>) -> Array<LrcPtr<(T, T)>> {
        if (get_Count(source.clone())) < 2_i32 {
            let a: Array<LrcPtr<(T, T)>> = new_empty::<LrcPtr<(T, T)>>();
            a
        } else {
            let len: i32 = (get_Count(source.clone())) - 1_i32;
    ...
    pub fn reduce<T: Clone + 'static>(reduction: Func2<T, T, T>, source: Array<T>) -> T {
        ...
        }
        {
            ...
            for i_1 in 0_i32..=(get_Count(source.clone())) - 1_i32 {
                acc_1.set({
    ...
    pub fn scan<T: Clone + 'static, State: Clone + 'static>(
    ...
    pub fn skip<T: Clone + 'static>(count: i32, source: Array<T>) -> Array<T> {
        if (count) > (get_Count(source.clone())) {
            panic!(
                "{}",
                append_1((outOfRange()), string(" (Parameter \'count\')")),
            );
        }
        {
            let count_1: i32 = if (count) < 0_i32 { 0_i32 } else { count };
            Array_::getSubArray(source.clone(), count_1, (get_Count(source)) - (count_1))
        }
    }
    pub fn skipWhile<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> Array<T> {
        let count: MutCell<i32> = MutCell::new(0_i32);
        while if (count.get().clone()) < (get_Count(source.clone())) {
        ...
        Array_::getSubArray(
            source.clone(),
            count.get().clone(),
            (get_Count(source.clone())) - (count.get().clone()),
        )
    }
    ...
    pub fn tryFind<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> Option<T> {
        fn inner_loop<T: Clone + 'static>(
            ...
            '_inner_loop: loop {
                break '_inner_loop (if (i.get().clone()) >= (get_Count(source_1.get().clone())) {
                    None::<T>
                } else {
    ...
    pub fn indexOf<T: core::hash::Hash + PartialEq + Clone + 'static>(
        source: Array<T>,
        item: T,
    ) -> i32 {
    ...
    pub fn tryFindBack<T: Clone + 'static>(
        ...
        inner_loop((get_Count(source.clone())) - 1_i32, predicate, source)
    }
    ...
    pub fn fold2<State: Clone + 'static, T1: Clone + 'static, T2: Clone + 'static>(
        ...
        for i in 0_i32..=(get_Count(source1.clone())) - 1_i32 {
            acc.set(folder(
                acc.get().clone(),
                source1[i].clone(),
                source2[i].clone(),
    ...
    pub fn permute<T: Clone + 'static>(indexMap: Func1<i32, i32>, source: Array<T>) -> Array<T> {
        let len: i32 = get_Count(source.clone());
        let res: Array<T> = toArray(source.clone());
    ...
    pub fn getSlice<T: Clone + 'static>(
        ...
        let patternInput: LrcPtr<(i32, i32)> = {
            ...
            let length: i32 = get_Count(source.clone());
            LrcPtr::new((
                if lower_1.is_some() && (lower_1.clone().unwrap()) >= 0_i32 {
                    let n_1: i32 = lower_1.clone().unwrap();
                    n_1
                } else {
                    0_i32
                },
                if upper_1.is_some() && (upper_1.clone().unwrap()) < (0_i32 + (length)) {
                    let m_1: i32 = upper_1.clone().unwrap();
                    m_1
                } else {
                    (0_i32 + (length)) - 1_i32
                },
    ...
    pub fn setSlice<T: Clone + 'static>(
        ...
        let count: i32 =
            ((defaultArg(upper, (get_Count(target.clone())) - 1_i32)) - (start)) + 1_i32;
        for i in 0_i32..=(count) - 1_i32 {
    ...
    pub fn sortDescending<T: PartialOrd + Clone + 'static>(source: Array<T>) -> Array<T> {
        Array_::sortWith(
            Func2::new(move |x: T, y: T| (compare(x, y)) * -1_i32),
            source,
        )
    }
    ...
    pub fn allPairs<T1: Clone + 'static, T2: Clone + 'static>(
        ...
        let len1: i32 = get_Count(xs.clone());
        let len2: i32 = get_Count(ys.clone());
    ...
    pub fn unfold<T: Clone + 'static, State: Clone + 'static>(
    ...
    pub fn chunkBySize<T: Clone + 'static>(chunkSize: i32, source: Array<T>) -> Array<Array<T>> {
        if (chunkSize) <= 0_i32 {
            panic!(
                "{}",
                append_1((inputMustBePositive()), string(" (Parameter \'size\')")),
            );
        }
        {
            let len: i32 = get_Count(source.clone());
    ...
    pub fn splitAt<T: Clone + 'static>(
        ...
        LrcPtr::new((
            ...
            Array_::getSubArray(source.clone(), index, (get_Count(source)) - (index)),
        ))
    }
    pub fn sum<T: core::ops::Add<Output = T> + Default + Clone + 'static>(source: Array<T>) -> T {
        let acc: MutCell<T> = MutCell::new(defaultOf());
        for i in 0_i32..=(get_Count(source.clone())) - 1_i32 {
    ...
    pub fn sumBy<T: Clone + 'static, U: core::ops::Add<Output = U> + Default + Clone + 'static>(
        ...
        let acc: MutCell<U> = MutCell::new(defaultOf());
        for i in 0_i32..=(get_Count(source.clone())) - 1_i32 {
    ...
    >(
        ...
        {
            let total: MutCell<T> = MutCell::new(defaultOf());
            for i in 0_i32..=(get_Count(source.clone())) - 1_i32 {
                total.set((total.get().clone()) + (source[i].clone()));
            }
            (total.get().clone()) / (T::from(get_Count(source.clone())))
        }
    }
    pub fn averageBy<
        T: Clone + 'static,
    ...
    >(
        ...
        {
            let total: MutCell<U> = MutCell::new(defaultOf());
            for i in 0_i32..=(get_Count(source.clone())) - 1_i32 {
                total.set((total.get().clone()) + (projection(source[i].clone())));
            }
            (total.get().clone()) / (U::from(get_Count(source.clone())))
        }
    }
    pub fn ofOption<T: Clone + 'static>(opt: Option<T>) -> Array<T> {
        match &opt {
            None => new_init(&getZero(), 0_i32),
    ...
    pub fn windowed<T: Clone + 'static>(windowSize: i32, source: Array<T>) -> Array<Array<T>> {
        ...
        {
            let len: i32 = 0_i32.max(((get_Count(source.clone())) - (windowSize)) + 1_i32);
    ...
    pub fn splitInto<T: Clone + 'static>(chunks: i32, source: Array<T>) -> Array<Array<T>> {
        if (chunks) <= 0_i32 {
            panic!(
                "{}",
                append_1((inputMustBePositive()), string(" (Parameter \'chunks\')")),
            );
        }
        if source.is_empty() {
            ...
            let chunks_1: i32 = chunks.min(get_Count(source.clone()));
            let minChunkSize: i32 = (get_Count(source.clone())) / (chunks_1);
            let chunksWithExtraItem: i32 = (get_Count(source.clone())) % (chunks_1);
    ...
    pub fn transpose<T: Clone + 'static>(arrays: Array<Array<T>>) -> Array<Array<T>> {
        if arrays.is_empty() {
            ...
            let len: i32 = get_Count(arrays.clone());
            let innerLen: i32 = get_Count(arrays[0_i32].clone());
            if !Array_::forAll(
                Func1::new({
                    let innerLen = innerLen.clone();
                    move |a_1: Array<T>| (get_Count(a_1)) == (innerLen)
                }),
                arrays.clone(),
            ) {
                panic!("{}", arraysHadDifferentLengths(),);
    ...
    pub fn distinct<T: core::hash::Hash + PartialEq + Clone + 'static>(xs: Array<T>) -> Array<T> {
    ...
    pub fn except<T: core::hash::Hash + PartialEq + Clone + 'static>(
        ...
        let hashSet: HashSet<T> = new_from_enumerable(itemsToExclude);
        Array_::filter(
            Func1::new({
                let hashSet = hashSet.clone();
                move |x: T| add_1(hashSet.clone(), x)
    ...
    pub fn countBy<T: Clone + 'static, Key: core::hash::Hash + PartialEq + Clone + 'static>(
        ...
        for idx in 0_i32..=(get_Count(xs.clone())) - 1_i32 {
        ...
        for idx in 0_i32..=(get_Count(xs.clone())) - 1_i32 {
            ...
            if matchValue.0.clone() {
                ...
                add_2(
                    dict.clone(),
                    key.clone(),
                    new_from_enumerable_1(ofArray(new_array(&[x]))),
                );
                add(keys.clone(), key)
            }
        }
    ...
    pub fn insertAt<T: Clone + 'static>(index: i32, y: T, xs: Array<T>) -> Array<T> {
        let len: i32 = get_Count(xs.clone());
        if if (index) < 0_i32 {
            true
        } else {
            (index) > (len)
    ...
    pub fn insertManyAt<T: Clone + 'static>(
        ...
        {
            let ys_1: Array<T> = toArray_1(ys);
            let len2: i32 = get_Count(ys_1.clone());

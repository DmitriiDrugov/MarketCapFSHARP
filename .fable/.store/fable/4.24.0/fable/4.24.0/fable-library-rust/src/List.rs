pub mod List_ {
    ...
    use crate::Global_::SR::inputListWasEmpty;
    ...
    use crate::Global_::SR::listsHadDifferentLengths;
    ...
    impl<T: Clone + 'static> core::fmt::Debug for Node_1<T> {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            write!(f, "{}", core::any::type_name::<Self>())
        }
    }
    impl<T: Clone + 'static> core::fmt::Display for Node_1<T> {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            write!(f, "{}", core::any::type_name::<Self>())
        }
    }
    ...
    impl<T: Clone + 'static> core::fmt::Debug for List<T> {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            write!(f, "{}", core::any::type_name::<Self>())
        }
    }
    impl<T: Clone + 'static> core::fmt::Display for List<T> {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            write!(f, "{}", core::any::type_name::<Self>())
        }
    }
    ...
    pub fn head<T: Clone + 'static>(xs: List_::List<T>) -> T {
        ...
        match &matchValue {
            None => panic!(
                "{}",
                append_1((inputListWasEmpty()), string(" (Parameter \'list\')")),
            ),
            ...
            break '_forAll2 ({
                ...
                let matchValue_1: bool = List_::isEmpty(ys.get().clone());
                {
                    ...
                    match &matchResult {
                        ...
                        1_i32 => {
                            if predicate(
                                List_::head(xs.get().clone()),
                                List_::head(ys.get().clone()),
                            ) {
                                let predicate_temp = predicate.get().clone();
    ...
    pub fn unfold<State: Clone + 'static, T: Clone + 'static>(
        ...
        while acc.get().clone().is_some() {
            let patternInput: LrcPtr<(T, State)> = acc.get().clone().unwrap();
    ...
    pub fn ofArray<T: Clone + 'static>(xs: Array<T>) -> List_::List<T> {
        List_::ofArrayWithTail(xs, List_::empty::<T>())
    }
    ...
    pub fn concat<T: Clone + 'static>(sources: List_::List<List_::List<T>>) -> List_::List<T> {
        ...
        let ys: MutCell<List_::List<T>> = MutCell::new(List_::empty::<T>());
    ...
    pub fn equals<T: PartialEq + Clone + 'static>(xs: List_::List<T>, ys: List_::List<T>) -> bool {
        let xs: MutCell<List_::List<T>> = MutCell::new(xs.clone());
        let ys: MutCell<List_::List<T>> = MutCell::new(ys.clone());
        '_equals: loop {
            break '_equals ({
    ...
    pub fn collect<T: Clone + 'static, U: Clone + 'static>(
        ...
        let node: MutCell<Option<LrcPtr<List_::Node_1<U>>>> = MutCell::new(root.get().clone());
        let xs_1: MutCell<List_::List<T>> = MutCell::new(xs);
        let ys: MutCell<List_::List<U>> = MutCell::new(List_::empty::<U>());
        while !List_::isEmpty(xs_1.get().clone()) {
    ...
    pub fn mapFoldBack<T: Clone + 'static, State: Clone + 'static, U: Clone + 'static>(
        ...
        let ys: LrcPtr<MutCell<List_::List<U>>> = LrcPtr::new(MutCell::new(List_::empty::<U>()));
    ...
    pub fn unzip<a: Clone + 'static, b: Clone + 'static>(
        ...
        List_::foldBack(
            ...
            xs,
            LrcPtr::new((List_::empty::<a>(), List_::empty::<b>())),
        )
    }
    ...
    pub fn sum<T: core::ops::Add<Output = T> + Default + Clone + 'static>(xs: List_::List<T>) -> T {
        List_::fold(Func2::new(move |acc: T, x: T| (acc) + (x)), defaultOf(), xs)
    }
    ...
    >(
        ...
        }
        {
            ...
            (List_::fold(folder, defaultOf(), xs)) / (T::from(count.get().clone()))
        }
    }
    pub fn averageBy<
        T: Clone + 'static,
    ...
    >(
        ...
        }
        {
            ...
            (List_::fold(folder, defaultOf(), xs)) / (U::from(count.get().clone()))
        }
    }
        ...
        '_skip: loop {
            break '_skip (if (count.get().clone()) <= 0_i32 {
                ...
                if List_::isEmpty(xs.get().clone()) {
                    panic!(
                        "{}",
                        append_1((notEnoughElements()), string(" (Parameter \'list\')")),
                    );
                }
                {
    ...
    pub fn getSlice<T: Clone + 'static>(
        ...
        if lower.is_some() {
            if upper.is_some() {
                let start_1: i32 = lower.clone().unwrap();
                let stop_1: i32 = upper.clone().unwrap();
                List_::truncate(
                    ((stop_1) - (start_1)) + 1_i32,
                    List_::skipSafe(start_1, xs.clone()),
                )
            } else {
                let start: i32 = lower.clone().unwrap();
                List_::skipSafe(start, xs.clone())
            }
        } else {
            if upper.is_some() {
                let stop: i32 = upper.clone().unwrap();
    ...
    pub fn splitAt<T: Clone + 'static>(
        ...
        if (index) < 0_i32 {
            panic!(
                "{}",
                append_1((inputMustBeNonNegative()), string(" (Parameter \'index\')")),
            );
        }
    ...
    pub fn exactlyOne<T: Clone + 'static>(xs: List_::List<T>) -> T {
        if List_::isEmpty(xs.clone()) {
            panic!(
                "{}",
                append_1((inputSequenceEmpty()), string(" (Parameter \'list\')")),
            )
        } else {
            if List_::isEmpty(List_::tail(xs.clone())) {
                ...
                panic!(
                    "{}",
                    append_1((inputSequenceTooLong()), string(" (Parameter \'list\')")),
                )
            }
        }
    }
            ...
            ));
            {
                let action_1 = Func1::new({
                    ...
                    move |xs_5: List_::List<T>| {
                        ...
                        if (len.get().clone()) != (get_Count(nodes.clone())) {
                            panic!(
                                "{}",
                                append_1(
                                    (listsHadDifferentLengths()),
    ...
    pub fn insertAt<T: Clone + 'static>(index: i32, y: T, xs: List_::List<T>) -> List_::List<T> {
        ...
        let res: List_::List<T> = {
            ...
            List_::fold(folder, List_::empty::<T>(), xs)
        };

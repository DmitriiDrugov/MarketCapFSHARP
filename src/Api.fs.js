...
import { uncurry } from "./fable_modules/fable-library.4.0.0/Util.js";
...
import { printf, toConsole } from "./fable_modules/fable-library.4.0.0/String.js";
...
export function getCoins() {
    return singleton.Delay(() => singleton.Bind(Http_get("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&sparkline=true"), (_arg) => {
        ...
        if (statusCode === 200) {
            const matchValue = fromString(uncurry(2, decoderList), _arg[1]);
            if (matchValue.tag === 1) {
                const clo = toConsole(printf("Decode error: %s"));
                clo(matchValue.fields[0]);
            ...
            const clo_1 = toConsole(printf("HTTP error: %d"));
            clo_1(statusCode);
            return singleton.Return(void 0);
        }
    }));
